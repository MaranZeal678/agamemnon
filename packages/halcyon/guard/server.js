/*
 * The Agamemnon guard server for Halcyon. Started when the plugin is installed.
 * localhost-only, permissive CORS. The embedded terminal talks to this.
 */
"use strict";
...
function createGuardServer(store, port) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      if (p === "/health") return send(res, 200, { ok: true, service: "agamemnon-guard", app: "halcyon", records: store.counts().total, installed: store.isInstalled(), enforcement: store.settings().enforcement, uptimeMs: Date.now() - started });
      if (p === "/counts") return send(res, 200, store.counts());
      if (p === "/stats") return send(res, 200, store.stats());
      if (p === "/records") { const q = u.searchParams; return send(res, 200, { ...store.list({ status: q.get("status"), region: q.get("region"), q: q.get("q"), limit: Number(q.get("limit") || 25), offset: Number(q.get("offset") || 0) }), counts: store.counts() }); }
      ...
      if (req.method === "POST") {
        ...
        if (p === "/install") { store.install(); return send(res, 200, store.settings()); }
        if (p === "/reset") { store.resetDemo(); return send(res, 200, { ok: true }); }
      }
      send(res, 404, { error: "not found" });
    } catch (e) { send(res, 400, { error: e && e.message ? e.message : String(e) }); }
  });
  return new Promise((resolve) => {
    server.on("error", (e) => { if (e.code === "EADDRINUSE") resolve({ server: null, port, inUse: true }); else resolve({ server: null, error: e }); });
    server.listen(port, "127.0.0.1", () => resolve({ server, port, inUse: false }));
  });
}
